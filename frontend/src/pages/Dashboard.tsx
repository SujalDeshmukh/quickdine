...
export default function Dashboard() {
    ...
    useEffect(() => {
        const fetchBookings = async () => {
            try {
                // GET /api/bookings/my — returns all bookings for the logged-in user.
                // The token is automatically sent via the Authorization header set in AppContext.
    ...
    useEffect(() => {
        const fetchRecommendations = async () => {
            try {
                // GET /api/restaurants/featured — returns top-rated featured restaurants
                const { data } = await axios.get("http://localhost:5000/api/restaurants/featured");
    ...
    const handleCancelBooking = async (bookingId: string) => {
        ...
        try {
            // PATCH /api/bookings/:id/cancel — updates status to 'cancelled'.
            // The backend verifies ownership (ensures users can't cancel others' bookings).
