...
export default function RestaurantDetail() {
    ...
    useEffect(() => {
        const fetchRestaurant = async () => {
            try {
                // GET /api/restaurants/:slug — fetch the full restaurant details by slug
                const { data } = await axios.get(`http://localhost:5000/api/restaurants/${slug}`);
                setRestaurant(data.data);
            } catch {
                // If the restaurant isn't found, navigate away
                navigate("/search");
            } finally {
                setLoading(false);
            }
    ...
    useEffect(() => {
        const fetchAvailability = async () => {
            ...
            try {
                ...
                // The backend's getSlotAvailability controller uses .reduce() to calculate
                // available seats per slot in real-time based on existing confirmed bookings.
                const { data } = await axios.get(
                    `http://localhost:5000/api/restaurants/${restaurant._id}/availability?date=${selectedDate}`
                );
                setSlotsAvailability(data.data);
            } catch {
